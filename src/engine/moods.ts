...
function getRelevantEffects(): { [modifier: string]: Effect[] } {
  ...
  if (have($item`ultra-soft ferns`)) result["-combat"].push($effect`Ultra-Soft Steps`);
  if (have($item`crunchy brush`)) result["+combat"].push($effect`Crunchy Steps`);
